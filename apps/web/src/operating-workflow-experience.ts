...
type Credential = { id: string; activity_id: string; status?: string; issuer_name?: string; issued_at?: string | null };
type RegistryEvent = { id: string; credential_id: string; event_type: string; created_at?: string };
type Settlement = { id: string; credential_id?: string | null; status?: string; created_at?: string; settled_at?: string | null };
type Workspace = { data?: { activities?: Activity[]; measurements?: Measurement[]; evidence?: Evidence[]; verifications?: Verification[] } };
type RegistryWorkspace = { data?: { credentials?: Credential[]; events?: RegistryEvent[] } };
...
type StageStatus = "READY" | "PENDING" | "VERIFIED" | "CONFIRMED" | "BLOCKED" | "NOT_REACHED" | "UNAVAILABLE";
type Stage = { title: string; detail: string; target: string; status: StageStatus };
...
function stagesFor(context: OperatingContext, activity: Activity | null, measurements: Measurement[], evidence: Evidence[], verifications: Verification[], provenance: Provenance[], credentials: Credential[], settlements: Settlement[], registryAvailable: boolean, settlementAvailable: boolean): Stage[] {
  ...
  const credential = credentials.some((c) => c.activity_id === id && ["ISSUED", "ACTIVE", "TRANSFERRED", "RETIRED"].includes(String(c.status).toUpperCase()));
  const credentialActive = credentials.some((c) => c.activity_id === id && ["ACTIVE", "TRANSFERRED"].includes(String(c.status).toUpperCase()));
  const settlement = settlements.some((s) => s.credential_id && credentials.some((c) => c.id === s.credential_id) && ["CREATED", "AUTHORIZED", "EXECUTING", "RECONCILING", "SETTLED"].includes(String(s.status).toUpperCase()));
  ...
  return [
    ...
    { title: "8 · Value / Registry", detail: registryAvailable ? (credential ? "An authoritative credential exists for this activity; registry state is shown from PostgreSQL." : hcs ? "HCS is confirmed, but no credential exists yet. Registry issuance is a separate guarded action." : "Registry is not reached because the upstream MRV/provenance gates are incomplete.") : "Registry workspace is unavailable or not authorized; readiness is not inferred.", target: targets[7], status: !registryAvailable ? "UNAVAILABLE" : credential ? "CONFIRMED" : hcs ? "PENDING" : "NOT_REACHED" },
    { title: "9 · Settlement", detail: settlementAvailable ? (settlement ? "An authoritative settlement exists for a credential in this transaction." : credentialActive ? "Credential is registry-active, but no settlement exists yet." : "Settlement is not reached until an authoritative active/transfer state exists.") : "Settlement workspace is unavailable or not authorized; readiness is not inferred.", target: targets[8], status: !settlementAvailable ? "UNAVAILABLE" : settlement ? "CONFIRMED" : credentialActive ? "PENDING" : "NOT_REACHED" },
  ];
}
function render(context: OperatingContext = readOperatingContext(), state?: { workspace?: Workspace; registry?: RegistryWorkspace; settlement?: SettlementWorkspace; provenance: Provenance[]; statutory?: Statutory; registryAvailable?: boolean; settlementAvailable?: boolean; message?: string }): void {
...
async function load(context: OperatingContext = currentContext): Promise<void> {
  ...
  try {
    const workspace = await api<Workspace>("/api/v1/workspaces/mrv", token); const activity = workspace.data?.activities?.[0];
    let provenance: Provenance[] = []; if (activity?.id) provenance = (await api<{ provenance?: Provenance[] }>(`/api/v1/mrv/provenance/${encodeURIComponent(activity.id)}`, token).catch(() => ({ provenance: [] }))).provenance ?? [];
    const statutory = await api<Statutory>("/api/v1/statutory/applicability", token).catch(() => undefined);
    const registryResult = await api<RegistryWorkspace>("/api/v1/workspaces/registry", token).then((registry) => ({ registry, available: true })).catch(() => ({ registry: undefined, available: false }));
    const settlementResult = await api<SettlementWorkspace>("/api/v1/workspaces/settlement", token).then((settlement) => ({ settlement, available: true })).catch(() => ({ settlement: undefined, available: false }));
    render(context, { workspace, provenance, statutory, registry: registryResult.registry, settlement: settlementResult.settlement, registryAvailable: registryResult.available, settlementAvailable: settlementResult.available });
  } catch (error) { render(context, { provenance: [], registryAvailable: false, settlementAvailable: false, message: error instanceof Error ? error.message : "Authoritative operating state unavailable." }); }
}
function mount(): void { render(currentContext, { provenance: [], registryAvailable: false, settlementAvailable: false, message: "Loading authoritative transaction state…" }); void load(currentContext); window.addEventListener("rupaykg:operating-context-change", (event) => { currentContext = (event as CustomEvent<OperatingContext>).detail === "rural" ? "rural" : "urban"; void load(currentContext); }); window.addEventListener("rupaykg:session-ready", () => void load(currentContext)); window.addEventListener("storage", (event) => { if (event.key === ORG_KEY) void load(currentContext); }); window.setInterval(() => { if (readSessionToken()) void load(currentContext); }, 60000); }
