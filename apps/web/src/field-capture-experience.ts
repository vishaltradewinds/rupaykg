...
function geographyLabel(g: Geography, depth: number): string { return `${"\u00a0\u00a0".repeat(Math.min(depth, 8))}${g.name || g.code || g.id}${g.kind ? ` · ${g.kind}` : ""}`; }
function appendGeographyOptions(select: HTMLSelectElement): void {
  const byParent = new Map<string, Geography[]>();
  const roots: Geography[] = [];
  for (const g of geographies) { const parent = g.parent_id || ""; if (!parent) roots.push(g); else { const list = byParent.get(parent) || []; list.push(g); byParent.set(parent, list); } }
  const add = (g: Geography, depth: number, seen: Set<string>) => { if (seen.has(g.id)) return; seen.add(g.id); const option = el("option", geographyLabel(g, depth)); option.value = g.id; select.appendChild(option); for (const child of (byParent.get(g.id) || []).slice().sort((a,b) => String(a.name || a.code).localeCompare(String(b.name || b.code)))) add(child, depth + 1, seen); };
  const seen = new Set<string>();
  roots.slice().sort((a,b) => String(a.name || a.code).localeCompare(String(b.name || b.code))).forEach(g => add(g, 0, seen));
  if (!roots.length) geographies.slice().sort((a,b) => String(a.name || a.code).localeCompare(String(b.name || b.code))).forEach(g => add(g, 0, seen));
}
...
async function loadAuthoritativeInputs() {
  ...
  if (devicesResponse.ok) { const body = await devicesResponse.json().catch(() => ({})); const devices = Array.isArray(body.devices) ? body.devices : []; const mine = devices.find((d: any) => d?.identity_id === identityId && d?.status === "VERIFIED"); verifiedDeviceId = String(mine?.id || ""); }
  if (geographyResponse.ok) { const body = await geographyResponse.json().catch(() => ({})); geographies = Array.isArray(body?.data?.geography) ? body.data.geography : []; }
}
async function submitAuthoritative(draft: Draft): Promise<{ queued: boolean; synced: boolean; entityId?: string }> {
  ...
  if (!draft.geographyId) throw new Error("Select an authorized operational geography before submitting.");
...
function render(context: OperatingContext = readOperatingContext()) {
  const config = getOperatingContextConfig(context); const shell = document.querySelector(".app-shell"); if (!shell) return;
  ...
  const geography = el("select"); geography.required = true; geography.setAttribute("aria-label", "Authorized operational geography"); const placeholder = el("option", geographies.length ? "Select authorized operational geography" : "Sign in and select an authorized geography"); placeholder.disabled = true; placeholder.value = ""; geography.appendChild(placeholder); geography.value = ""; appendGeographyOptions(geography);
  const geographyHint = el("div", "Only organization-authorized geography is selectable. Use the most specific available level (for example ward, village or facility). "); geographyHint.style.cssText = "font-size:11px;opacity:.72;margin-top:-5px";
  ...
  form.append(activity, material, quantity, unit, geography, geographyHint, notes, save, status); panel.appendChild(form);
  form.addEventListener("submit", event => { event.preventDefault(); const selectedOption = geography.options[geography.selectedIndex]; const draft: Draft = { id: crypto.randomUUID(), context, activityType: activity.value, material: material.value, quantity: quantity.value, unit: unit.value, geography: selectedOption?.textContent || "", geographyId: geography.value || undefined, notes: notes.value.trim(), capturedAt: new Date().toISOString(), syncStatus: "LOCAL_DRAFT" }; if (!draft.quantity || Number(draft.quantity) <= 0) { status.textContent = "Enter a positive quantity."; return; } saveDraft(draft); void (async () => { try { const result = await submitAuthoritative(draft); draft.syncStatus = result.synced ? "SYNCED" : result.queued ? "QUEUED" : "LOCAL_DRAFT"; saveDraft(draft); status.textContent = result.synced ? `Authoritative activity accepted${result.entityId ? ` · ${result.entityId}` : ""}.` : result.queued ? "Field operation queued for authoritative sync." : "Field draft saved."; quantity.value = ""; geography.value = ""; notes.value = ""; } catch (cause) { status.textContent = `${cause instanceof Error ? cause.message : "Submission failed"} Draft retained on this device.`; } })(); });
}
function mount() { render(); if (firebaseAuth) onAuthStateChanged(firebaseAuth, async user => { if (!user) { sessionToken = ""; verifiedDeviceId = ""; return; } try { await establishSession(user); await loadAuthoritativeInputs(); render(); } catch {} }); window.addEventListener("rupaykg:operating-context-change", event => render((event as CustomEvent<OperatingContext>).detail === "rural" ? "rural" : "urban")); }
