...
function Panel() {
  ...
  const nextStep = latest
    ? { title: "Review registry authority", detail: "The calculation is present. Credential and settlement state are intentionally not inferred here.", action: "registry" }
    : !activity ? { title: "Record an authoritative activity", detail: "No activity is linked to a carbon calculation for this organization.", action: "operations" }
    : !latestMeasurement ? { title: "Capture an authoritative measurement", detail: `Activity ${activity.id} has no linked measurement in the MRV workspace.`, action: "mrv & evidence" }
    : !latestEvidence ? { title: "Attach evidence to the measured activity", detail: `Measurement ${latestMeasurement.id} is present; no linked evidence record is observed.`, action: "mrv & evidence" }
    : !latestVerification ? { title: "Complete verification", detail: `Evidence ${latestEvidence.id} is present; no linked verification decision is observed.`, action: "mrv & evidence" }
    : { title: "Review carbon calculation", detail: "MRV records are present. Continue with the authoritative carbon workspace when calculation is available.", action: "carbon / value" };
  return <section className="ccc-shell" aria-label="Carbon and CCTS Command Center">
    ...
    <div className="ccc-next"><div><p className="eyebrow">NEXT AUTHORITATIVE STEP</p><strong>{nextStep.title}</strong><small>{nextStep.detail}</small></div><button type="button" onClick={() => jump(nextStep.action)}>Open {nextStep.action}</button></div>
