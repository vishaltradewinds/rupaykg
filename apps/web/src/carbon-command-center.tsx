...
function statusLabel(value: unknown) {
  ...
  if (["VERIFIED", "APPROVED", "ACTIVE", "ELIGIBLE", "CONFIRMED", "ISSUED"].includes(raw)) return "VERIFIED";
  if (["PENDING", "UNDER_REVIEW", "CALCULATED"].includes(raw)) return "PENDING";
  if (["REJECTED", "FAILED", "BLOCKED"].includes(raw)) return "REJECTED";
...
function Stage({ name, state, detail, onClick }: { name: string; state: string; detail: string; onClick?: () => void }) {
  return <button type="button" className={`ccc-stage state-${state.toLowerCase()}`} onClick={onClick} disabled={!onClick}>
    <span>{state}</span><strong>{name}</strong><small>{detail}</small>
  </button>;
}
...
function Panel() {
  ...
  const membership = React.useMemo(() => {
    const verified = me?.memberships.filter(m => m.status === "VERIFIED") ?? [];
    const stored = localStorage.getItem(orgKey) ?? "";
    return verified.find(m => m.organization_id === stored) ?? verified[0] ?? null;
  }, [me]);

  const load = React.useCallback(async (sessionToken: string) => {
    setLoading(true); setError("");
    try {
      const headers = new Headers({ Authorization: `Bearer ${sessionToken}`, Accept: "application/json" });
      const activeOrg = localStorage.getItem(orgKey);
      if (activeOrg) headers.set("X-RupayKG-Organization-Id", activeOrg);
      const [identityResponse, carbonResponse] = await Promise.all([fetch("/api/v1/auth/me", { headers }), fetch("/api/v1/workspaces/carbon", { headers })]);
      const identity = await identityResponse.json().catch(() => ({})) as Me;
      const carbon = await carbonResponse.json().catch(() => ({})) as Workspace;
      ...
      setMe(identity); setWorkspace(carbon);
  ...
  React.useEffect(() => {
    ...
    return onAuthStateChanged(firebaseAuth, async currentUser => {
      setUser(currentUser);
      if (!currentUser || !currentUser.emailVerified) { setToken(""); setMe(null); setWorkspace(null); setLoading(false); return; }
      try { const sessionToken = await exchange(currentUser); setToken(sessionToken); await load(sessionToken); }
      catch (cause) { setError(cause instanceof Error ? cause.message : "Authenticated session unavailable."); setLoading(false); }
    });
  }, [load]);

  React.useEffect(() => {
    const refresh = () => { if (token) void load(token); };
    window.addEventListener("rupaykg:authoritative-mutation", refresh);
    window.addEventListener("storage", refresh);
    const timer = window.setInterval(refresh, 30000);
    return () => { window.removeEventListener("rupaykg:authoritative-mutation", refresh); window.removeEventListener("storage", refresh); window.clearInterval(timer); };
  }, [token, load]);

  if (!user || !token || loading) return null;
  if (error) return <section className="ccc-shell"><div className="ccc-error">{error}</div></section>;
  if (!membership) return null;

  const calculations = workspace?.calculations ?? [];
  const latest = calculations[0];
  const calculationState = latest ? statusLabel(latest.status) : "UNAVAILABLE";
  const methodologyState = latest?.methodology_code ? statusLabel(latest.governance_status) : "UNAVAILABLE";
  const valueState = latest ? statusLabel(latest.status) : "UNAVAILABLE";
  const verifiedCount = calculations.filter(c => statusLabel(c.status) === "VERIFIED").length;
  const pendingCount = calculations.filter(c => statusLabel(c.status) === "PENDING").length;

  const jump = (label: string) => {
    const button = Array.from(document.querySelectorAll("button")).find(b => b.textContent?.trim().toLowerCase().includes(label.toLowerCase()));
    button?.click();
  };

  return <section className="ccc-shell" aria-label="Carbon and CCTS Command Center">
    <div className="ccc-heading">
      <div><p className="eyebrow">CARBON / CCTS COMMAND CENTER</p><h2>Verified value pathway</h2><p>One operational view of the evidence-backed carbon pathway for the active organization.</p></div>
      <div className="ccc-context"><strong>{membership.organization_name}</strong><span>{membership.role_name}</span><small>Source: PostgreSQL · Synthetic data: false</small></div>
    </div>
    ...
    {latest ? <div className="ccc-detail-grid">
      <article className="ccc-card"><p className="eyebrow">LATEST AUTHORITATIVE CALCULATION</p><h3>{display(latest.result)} {latest.unit ?? ""}</h3><div className="ccc-fields"><span><b>Activity</b>{display(latest.activity_id)}</span><span><b>Methodology</b>{display(latest.methodology_code)} · {display(latest.methodology_version)}</span><span><b>Calculated</b>{display(latest.calculated_at)}</span><span><b>Status</b>{statusLabel(latest.status)}</span></div></article>
      <article className="ccc-card"><p className="eyebrow">PROVENANCE</p><div className="ccc-fields"><span><b>Dataset hash</b>{shortHash(latest.dataset_hash)}</span><span><b>Formula hash</b>{shortHash(latest.formula_hash)}</span><span><b>Calculation hash</b>{shortHash(latest.calculation_hash)}</span><span><b>Provenance version</b>{display(latest.provenance_version)}</span></div><small className="ccc-note">Hashes and trace are displayed as provenance references. They do not constitute government issuance, registry consensus, or settlement confirmation.</small></article>
    </div> : <div className="ccc-empty"><strong>No authoritative carbon calculation is available for this organization.</strong><span>Complete activity → measurement → evidence → approved verification → registered methodology before calculating value.</span></div>}
...
const mount = document.getElementById("carbon-command-center");
