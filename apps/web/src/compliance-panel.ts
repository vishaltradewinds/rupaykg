...
if (root && firebaseAuth) {
  const render = (state: PanelState) => {
    ...
    const sourceContext = state.eprSource
      ? `<aside class="compliance-source">
          <div>
            <p class="eyebrow">CURRENT CPCB SOURCE</p>
            <strong>${escapeHtml(state.eprSource.title)}</strong>
            <span>${escapeHtml(state.eprSource.authority)} · verified ${escapeHtml(formatDate(state.eprSource.verified_on))}</span>
          </div>
          ${sourceUrl ? `<a href="${escapeHtml(sourceUrl)}" target="_blank" rel="noreferrer">Open official portal</a>` : ""}
        </aside>`
      : `<div class="field-help">Current CPCB EPR source is not available from the authoritative regulatory catalog.</div>`;
  ...
  const load = async (user: User) => {
    try {
      ...
      const [workspace, sources] = await Promise.all([
        api<{ data: { obligations: Obligation[] } }>("/api/v1/workspaces/compliance", session.sessionToken),
      ...
      render({ status: "UNAVAILABLE", message: error instanceof Error ? error.message : "Unable to load authoritative compliance data.", obligations: [], canAssess: false, eprSource: null });
    }
  };

  onAuthStateChanged(firebaseAuth, (user) => {
    if (user) void load(user);
    else render({ status: "SIGN IN REQUIRED", message: "Sign in to load authorized compliance obligations.", obligations: [], canAssess: false, eprSource: null });
  });
}
