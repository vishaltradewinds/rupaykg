...
test("production lifecycle integration gate", { skip: !url }, async () => {
  ...
  try {
    ...
    await assert.rejects(() => c.query("insert into verifications(evidence_id,activity_id,verifier_identity_id,decision,scope) values($1,$2,$3,'APPROVED','integration')", [evidence, activity, actor]), /self-approve/);
    ...
    await assert.rejects(() => c.query("update settlements set status='SETTLED' where id=$1", [settlement]), /external settlement reference/);
    ...
    await assert.rejects(() => c.query("update settlements set external_confirmed_at=null where id=$1", [settlement]), /cannot be cleared/);
    await assert.rejects(() => c.query("update registry_events set event_type='TAMPERED' where credential_id=$1", [credential]), /append-only/);
