...
export async function registerWorkspaceRoutes(app: FastifyInstance, pool: Pool | null): Promise<void> {
  ...
  app.get("/api/v1/workspaces/carbon", async (request, reply) => {
    ...
    try{const params:unknown[]=[ids];if(geographyId)params.push(geographyId);const rows=await pool.query(`select cc.id,cc.activity_id,cc.methodology_version_id,mv.methodology_code,mv.version as methodology_version,cc.inputs,cc.result,cc.unit,cc.status,cc.calculated_at from carbon_calculations cc join activities a on a.id=cc.activity_id join methodology_versions mv on mv.id=cc.methodology_version_id where a.organization_id=any($1::uuid[])${geoClause(geographyId,"a.geography_id")} order by cc.calculated_at desc nulls last,cc.id desc limit 100`,params);return result({calculations:rows.rows});}
