...
function requireOrganization(auth: AuthContext, organizationId: string, reply: { code: (status: number) => { send: (body: unknown) => unknown } }): boolean {
  if (!canActForOrganization(auth, organizationId)) { reply.code(403).send({ error: "No verified membership for organization", code: "ORG_FORBIDDEN" }); return false; }
  return true;
}

app.get("/health", async (_request, reply) => {
  let database: "AVAILABLE" | "UNAVAILABLE" = "UNAVAILABLE";
  if (pool) { try { await pool.query("select 1"); database = "AVAILABLE"; } catch { database = "UNAVAILABLE"; } }
...
app.get("/api/v1/regulatory/sources", async (request, reply) => {
  try { return { source: "postgresql", syntheticData: false, sources: await query("select id, authority, title, instrument, reference, published_on, effective_from, jurisdiction, source_url, verified_on, status, affected_module, notes from regulatory_sources order by effective_from desc nulls last, published_on desc") }; }
  catch (error) { request.log.error(error); return reply.code(503).send({ error: "Regulatory source catalog unavailable", syntheticData: false }); }
});
app.get("/api/v1/geography/children/:parentId", async (request, reply) => {
  try { const { parentId } = request.params as { parentId: string }; return { source: "postgresql", syntheticData: false, geography: await query("select id, parent_id, kind, code, external_code, name, source, source_version, valid_from, valid_to, metadata from geography where parent_id = $1 order by name", [parentId]) }; }
  catch (error) { request.log.error(error); return reply.code(503).send({ error: "Authoritative geography unavailable", syntheticData: false }); }
});
app.post("/api/v1/operations/sync", async (request, reply) => {
  ...
  try {
    ...
    if (existing[0]) { if (existing[0].actor_identity_id !== auth.identityId) return reply.code(403).send({ error: "Idempotency key belongs to another identity", code: "IDEMPOTENCY_FORBIDDEN" }); return { source: "postgresql", syntheticData: false, replay: true, operation: existing[0] }; }
...
app.post("/api/v1/evidence/:evidenceId/verification", async (request, reply) => {
  const auth = await requireAuth(request, reply); if (!auth) return;
  const { evidenceId } = request.params as { evidenceId: string }; const body = bodyOf(request); const decision = requiredString(body, "decision"); const scope = requiredString(body, "scope");
  ...
  try {
    const verification = await withTransaction(async client => {
      ...
      const result = await client.query("insert into verifications (evidence_id, verifier_identity_id, decision, scope, rationale) values ($1,$2,$3,$4,$5) returning *", [evidenceId, auth.identityId, decision, scope, body.rationale ?? null]);
  ...
  } catch (error) { request.log.error(error); if ((error as { code?: string }).code === "VERIFICATION_FORBIDDEN") return reply.code(403).send({ error: "Verifier is not authorized for this evidence", code: "VERIFICATION_FORBIDDEN" }); return reply.code(503).send({ error: "Evidence verification unavailable", syntheticData: false }); }
});
