...
after(async () => {
  ...
  try {
    ...
    await c.query("delete from organizations where id in ($1,$2)", [ownerOrgId, destinationOrgId]);
...
describe("registry and settlement runtime acceptance", () => {
  it("requires explicit registry permission before issuance", async () => {
    ...
    await pool.query("delete from identity_sessions where identity_id=$1", [deniedIdentity]);
    await pool.query("delete from organization_memberships where identity_id=$1", [deniedIdentity]);
    await pool.query("delete from roles where id=$1", [deniedRole]);
    await pool.query("delete from identities where id=$1", [deniedIdentity]);
  });
