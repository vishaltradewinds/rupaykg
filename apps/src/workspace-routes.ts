...
function scopedGeoClause(column: string): string { return ` and (${column} is null or organization_has_geography_scope(${column === "a.geography_id" ? "a.organization_id" : column === "ob.jurisdiction_id" ? "ob.organization_id" : column === "rf.source_geography_id" ? "rf.organization_id" : "a.organization_id"}, ${column}))`; }
...
export async function registerWorkspaceRoutes(app: FastifyInstance, pool: Pool | null): Promise<void> {
  ...
  app.get("/api/v1/workspaces/mrv", async (request, reply) => {
    ...
    try{const params:unknown[]=[ids];if(geographyId)params.push(geographyId);const clause=geoClause(geographyId,"a.geography_id");const scopeClause=" and (a.geography_id is null or organization_has_geography_scope(a.organization_id,a.geography_id))";const [activities,measurements,evidence,verifications]=await Promise.all([
      ...
      pool.query(`select v.id,v.evidence_id,v.activity_id,v.verifier_identity_id,v.decision,v.scope,v.rationale,v.decided_at from verifications v join evidence e on e.id=v.evidence_id join activities a on a.id=e.activity_id where a.organization_id=any($1::uuid[])${scopeClause}${clause} order by v.decided_at desc limit 100`,params)]);return result({activities:activities.rows,measurements:measurements.rows,evidence:evidence.rows,verifications:verifications.rows});}
  ...
  app.get("/api/v1/workspaces/compliance", async (request, reply) => {
    ...
    try{const params:unknown[]=[ids];if(geographyId)params.push(geographyId);const clause=geoClause(geographyId,"ob.jurisdiction_id");const rows=await pool.query(`select ob.id,ob.organization_id,o.name as organization_name,ob.jurisdiction_id,g.name as jurisdiction_name,ob.obligation_type,ob.period_start,ob.period_end,ob.required_quantity,ob.status from obligations ob join organizations o on o.id=ob.organization_id left join geography g on g.id=ob.jurisdiction_id where ob.organization_id=any($1::uuid[]) and (ob.jurisdiction_id is null or organization_has_geography_scope(ob.organization_id,ob.jurisdiction_id))${clause} order by ob.period_end desc limit 100`,params);return result({obligations:rows.rows});}
  ...
  app.get("/api/v1/workspaces/carbon", async (request, reply) => {
    ...
    try{const params:unknown[]=[ids];if(geographyId)params.push(geographyId);const rows=await pool.query(`select cc.id,cc.activity_id,cc.methodology_version_id,mv.methodology_code,mv.version as methodology_version,mv.governance_status,cc.inputs,cc.result,cc.unit,cc.status,cc.calculated_at,cc.dataset_hash,cc.formula_hash,cc.calculation_hash,cc.provenance_version,cc.calculation_trace from carbon_calculations cc join activities a on a.id=cc.activity_id join methodology_versions mv on mv.id=cc.methodology_version_id where a.organization_id=any($1::uuid[]) and (a.geography_id is null or organization_has_geography_scope(a.organization_id,a.geography_id))${geoClause(geographyId,"a.geography_id")} order by cc.calculated_at desc nulls last,cc.id desc limit 100`,params);return result({calculations:rows.rows});}
  ...
  app.get("/api/v1/workspaces/registry", async (request, reply) => {
    ...
    try{const params:unknown[]=[ids];if(geographyId)params.push(geographyId);const clause=geoClause(geographyId,"a.geography_id");const scopeClause=" and (a.geography_id is null or organization_has_geography_scope(a.organization_id,a.geography_id))";const [credentials,events]=await Promise.all([
      pool.query(`select c.id,c.activity_id,c.issuer_organization_id,o.name as issuer_name,c.trust_root_id,c.status,c.credential_uri,c.issued_at from credentials c join organizations o on o.id=c.issuer_organization_id join activities a on a.id=c.activity_id left join lateral (select re.to_owner_id as current_owner_id from registry_events re where re.credential_id=c.id and re.event_type in('ISSUED','TRANSFERRED') order by re.created_at desc limit 1) current_owner on true where (c.issuer_organization_id=any($1::uuid[]) or current_owner.current_owner_id=any($1::uuid[]))${scopeClause}${clause} order by c.issued_at desc nulls last,c.id desc limit 100`,params),
      pool.query(`select re.id,re.credential_id,re.event_type,re.from_owner_id,fo.name as from_owner_name,re.to_owner_id,too.name as to_owner_name,re.external_reference,re.event_hash,re.created_at from registry_events re left join organizations fo on fo.id=re.from_owner_id left join organizations too on too.id=re.to_owner_id join credentials c on c.id=re.credential_id join activities a on a.id=c.activity_id left join lateral (select re2.to_owner_id as current_owner_id from registry_events re2 where re2.credential_id=c.id and re2.event_type in('ISSUED','TRANSFERRED') order by re2.created_at desc limit 1) current_owner on true where (c.issuer_organization_id=any($1::uuid[]) or current_owner.current_owner_id=any($1::uuid[]))${scopeClause}${clause} order by re.created_at desc limit 200`,params)]);return result({credentials:credentials.rows,events:events.rows});}
