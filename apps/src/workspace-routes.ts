...
export async function registerWorkspaceRoutes(app: FastifyInstance, pool: Pool | null): Promise<void> {
  ...
  app.post("/api/v1/evidence/:evidenceId/verification", async (request, reply) => {
    ...
    try {
      ...
      if (auth.activeOrganizationId) { const scopeCheck = await client.query<{ ok: boolean }>("select exists(select 1 from activities where id=$1 and organization_id=$2) as ok", [activityId, auth.activeOrganizationId]); if (!scopeCheck.rows[0]?.ok) { await client.query("ROLLBACK"); return reply.code(403).send({ error: "Evidence is outside the active organization scope", code: "ORGANIZATION_FORBIDDEN" }); } }
  ...
  app.get("/api/v1/workspaces/compliance", async (request, reply) => {
    ...
    try{const params:unknown[]=[ids];if(geographyId)params.push(geographyId);const clause=geoClause(geographyId,"ob.jurisdiction_id");const rows=await pool.query(`select ob.id,ob.organization_id,o.name as organization_name,ob.jurisdiction_id,g.name as jurisdiction_name,ob.obligation_type,ob.period_start,ob.period_end,ob.required_quantity,ob.status from obligations ob join organizations o on o.id=ob.organization_id left join geography g on g.id=ob.jurisdiction_id where ob.organization_id=any($1::uuid[]) and organization_has_geography_scope(ob.organization_id,ob.jurisdiction_id)${clause} order by ob.period_end desc limit 100`,params);return result({obligations:rows.rows});}
  ...
  app.get("/api/v1/workspaces/registry", async (request, reply) => {
    ...
    try{const params:unknown[]=[ids];if(geographyId)params.push(geographyId);const clause=geoClause(geographyId,"a.geography_id");const scopeClause=" and organization_has_geography_scope(a.organization_id,a.geography_id)";const [credentials,events]=await Promise.all([
