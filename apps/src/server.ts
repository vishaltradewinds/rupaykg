...
async function canWriteActivity(client: Pool | PoolClient, auth: AuthContext, activityId: string): Promise<boolean> { const result = await client.query<{ ok: boolean }>(`select exists(select 1 from activities a join organization_memberships om on om.organization_id=a.organization_id and om.identity_id=$2 and om.status='VERIFIED' where a.id=$1 and a.geography_id is not null and organization_has_geography_scope(om.organization_id,a.geography_id)) as ok`, [activityId, auth.identityId]); return result.rows[0]?.ok === true; }
async function canWriteGeography(client: Pool | PoolClient, auth: AuthContext, organizationId: string, geographyId: string): Promise<boolean> { if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(geographyId)) return false; const result = await client.query<{ ok: boolean }>(`select exists(select 1 from organization_memberships om where om.identity_id=$1 and om.organization_id=$2 and om.status='VERIFIED' and organization_has_geography_scope(om.organization_id,$3)) as ok`, [auth.identityId, organizationId, geographyId]); return result.rows[0]?.ok === true; }

app.get("/health", async (_request, reply) => { let database: "AVAILABLE" | "UNAVAILABLE" = "UNAVAILABLE"; if (pool) { try { await pool.query("select 1"); database = "AVAILABLE"; } catch {} } const healthy = database === "AVAILABLE"; return reply.code(healthy ? 200 : 503).send({ status: healthy ? "READY" : "DEGRADED", database, syntheticData: false }); });
...
app.get("/api/v1/overview", async (request, reply) => {
  ...
  try {
    ...
    const [activities, measurements, evidence, verifications, obligations, credentials, settlements] = await Promise.all([
      query<{ count: string }>("select count(*)::text as count from activities where organization_id = any($1::uuid[])", [organizationIds]),
      query<{ count: string }>("select count(*)::text as count from measurements m join activities a on a.id=m.activity_id where a.organization_id = any($1::uuid[])", [organizationIds]),
      query<{ count: string }>("select count(*)::text as count from evidence e join activities a on a.id=e.activity_id where a.organization_id = any($1::uuid[])", [organizationIds]),
...
app.get("/api/v1/regulatory/sources", async (_request, reply) => { try { return { source: "postgresql", syntheticData: false, sources: await query("select id, authority, title, instrument, reference, published_on, effective_from, jurisdiction, source_url, verified_on, status, affected_module, notes from regulatory_sources order by effective_from desc nulls last, published_on desc") }; } catch (error) { return reply.code(503).send({ error: "Regulatory source catalog unavailable", syntheticData: false }); } });
