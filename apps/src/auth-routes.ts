...
export async function registerAuthRoutes(app: FastifyInstance, pool: Pool | null): Promise<void> {
  app.post("/api/v1/auth/exchange", async (request, reply) => {
    ...
    try {
      ...
      if (!identityRow) throw new Error("Identity insert returned no row");
  ...
  app.post("/api/v1/onboarding/applications", async (request, reply) => {
    ...
    const roleKey = roleKeyText as StakeholderRoleKey;
    const option = STAKEHOLDER_OPTIONS.find(([key]) => key === roleKey);
    if (!option) return reply.code(400).send({ error: "Unsupported stakeholder role", code: "INVALID_ONBOARDING" });
    ...
    try {
      ...
      const organizationRow = organization.rows[0]; if (!organizationRow) throw new Error("Organization insert returned no row");
      const role = await client.query<{ id: string }>("insert into roles(organization_id,name,permissions,geography_scope) values($1,$2,'[]','[]') returning id", [organizationRow.id, roleKey]);
      const roleRow = role.rows[0]; if (!roleRow) throw new Error("Role insert returned no row");
  ...
  app.post("/api/v1/onboarding/applications/:applicationId/approve", async (request, reply) => { const auth = await requireAuth(request, reply, pool); if (!auth || !pool) return; const id = (request.params as { applicationId: string }).applicationId; const allowed = await pool.query<{ ok: boolean }>(`select exists(select 1 from organization_memberships om join roles r on r.id=om.role_id where om.identity_id=$1 and om.status='VERIFIED' and (r.permissions @> '["MANAGE_STAKEHOLDERS"]'::jsonb or r.name in('platform_admin','super_admin'))) ok`, [auth.identityId]); if (!allowed.rows[0]?.ok) return reply.code(403).send({ error: "Explicit MANAGE_STAKEHOLDERS permission is required", code: "STAKEHOLDER_APPROVAL_FORBIDDEN" }); const client = await pool.connect(); try { await client.query("begin"); const result = await client.query(`update stakeholder_applications set status='APPROVED',reviewed_by_identity_id=$1,reviewed_at=now() where id=$2 and status='PENDING' returning organization_id,role_id`, [auth.identityId, id]); const resultRow = result.rows[0]; if (!resultRow) { await client.query("rollback"); return reply.code(404).send({ error: "Pending stakeholder application not found", code: "APPLICATION_NOT_FOUND" }); } await client.query("update organization_memberships set status='VERIFIED' where organization_id=$1 and role_id=$2", [resultRow.organization_id, resultRow.role_id]); await client.query("update organizations set status='VERIFIED' where id=$1", [resultRow.organization_id]); await client.query("update organization_geography_scopes set status='VERIFIED' where organization_id=$1", [resultRow.organization_id]); await client.query("commit"); return { source: "postgresql", syntheticData: false, status: "APPROVED", organizationId: resultRow.organization_id }; } catch (error) { await client.query("rollback"); request.log.error(error); return reply.code(503).send({ error: "Stakeholder approval could not be finalized", code: "APPROVAL_UNAVAILABLE" }); } finally { client.release(); } });
}
