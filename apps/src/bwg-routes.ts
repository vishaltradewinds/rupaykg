...
function periodIdOf(request: { params: unknown }): string { const params = request.params && typeof request.params === "object" ? request.params as Record<string, unknown> : {}; return typeof params.periodId === "string" ? params.periodId : ""; }
...
export async function registerBwgRoutes(app: FastifyInstance, pool: Pool | null): Promise<void> {
  ...
  app.post("/api/v1/bwg/reporting-periods/:periodId/waste", async (request, reply) => {
    const auth = await authFor(pool, request, reply); if (!auth || !pool) return; const periodId = periodIdOf(request); const organizationId = await periodOrganization(auth, periodId, reply); if (!organizationId) return;
    ...
    try{const row=await pool.query("insert into bwg_waste_reports(reporting_period_id,waste_stream,generated_quantity,segregated_quantity,channelized_quantity,processed_quantity,unit,evidence_id,verification_id) values($1,$2,$3,$4,$5,$6,$7,$8,$9) returning *",[periodId,stream,generated,segregated,channelized,processed,unit,evidenceId,verificationId]);return reply.code(201).send({source:"postgresql",syntheticData:false,authoritativeMutation:true,report:row.rows[0]});}catch(error){request.log.error(error);return reply.code(503).send({error:"Waste report unavailable",syntheticData:false});}
  });

  app.post("/api/v1/bwg/reporting-periods/:periodId/epr", async (request, reply) => {
    const auth=await authFor(pool,request,reply);if(!auth||!pool)return;const periodId = periodIdOf(request);const organizationId=await periodOrganization(auth,periodId,reply);if(!organizationId)return;if(!await hasValuePermission(pool,"can_assess_epr",auth.identityId,organizationId))return reply.code(403).send({error:"EPR assessment permission required",code:"EPR_MANAGE_FORBIDDEN"});
    ...
    const evidenceId=str(body,"evidenceId");const verificationId=str(body,"verificationId");if(verificationId&&!evidenceId)return reply.code(400).send({error:"verificationId requires evidenceId",code:"VERIFICATION_EVIDENCE_REQUIRED"});if(evidenceId){const check=await verifiedEvidence(pool,evidenceId,verificationId,organizationId);if(!check.ok)return reply.code(check.code==="EVIDENCE_FORBIDDEN"?403:409).send({error:"Evidence or verification is not authoritative for this organization",code:check.code});}
    try{const row=await pool.query("insert into bwg_epr_reports(reporting_period_id,scheme_id,category_code,obligated_quantity,fulfilled_quantity,evidence_id,verification_id) values($1,$2,$3,$4,$5,$6,$7) returning *",[periodId,schemeId,categoryCode,obligated,fulfilled,evidenceId,verificationId]);return reply.code(201).send({source:"postgresql",syntheticData:false,authoritativeMutation:true,report:row.rows[0]});}catch(error){request.log.error(error);return reply.code(503).send({error:"EPR report unavailable",syntheticData:false});}
  });
  ...
  app.post("/api/v1/bwg/reporting-periods/:periodId/esg", async (request, reply) => {
    ...
    const evidenceId=str(body,"evidenceId");const verificationId=str(body,"verificationId");if(verificationId&&!evidenceId)return reply.code(400).send({error:"verificationId requires evidenceId",code:"VERIFICATION_EVIDENCE_REQUIRED"});if(evidenceId){const check=await verifiedEvidence(pool,evidenceId,verificationId,organizationId);if(!check.ok)return reply.code(check.code==="EVIDENCE_FORBIDDEN"?403:409).send({error:"Evidence or verification is not authoritative for this organization",code:check.code});}
    try{const row=await pool.query("insert into bwg_esg_reports(reporting_period_id,metric_code,scope,value,unit,evidence_id,verification_id) values($1,$2,$3,$4,$5,$6,$7) returning *",[periodId,metricCode,scope,metricValue,unit,evidenceId,verificationId]);return reply.code(201).send({source:"postgresql",syntheticData:false,authoritativeMutation:true,report:row.rows[0]});}catch(error){request.log.error(error);return reply.code(503).send({error:"ESG report unavailable",syntheticData:false});}
  });
}
