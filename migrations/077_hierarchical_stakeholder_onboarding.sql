...
declare gk geography_kind; levels text[]; level_name text; idx integer := 1;
begin
  select g.kind into gk from geography g where g.id=new.geography_id;
  levels := case
    when gk in ('ULB','WARD','LOCALITY','GRAM_PANCHAYAT','VILLAGE','CLUSTER') then array['LOCAL','DISTRICT','STATE','PLATFORM']
    when gk in ('SUB_DISTRICT','DISTRICT') then array['DISTRICT','STATE','PLATFORM']
    when gk='STATE_UT' then array['STATE','PLATFORM']
    else array['PLATFORM']
  end;
  foreach level_name in array levels loop
    ...
    values(new.id,idx,level_name,new.geography_id) on conflict do nothing;
    idx := idx+1;
  end loop;
  return new;
end;
...
do $$
declare r record; gk geography_kind; levels text[]; level_name text; idx integer;
begin
  for r in select sa.id,sa.geography_id from stakeholder_applications sa where sa.status='PENDING' loop
    select g.kind into gk from geography g where g.id=r.geography_id;
    levels := case
      when gk in ('ULB','WARD','LOCALITY','GRAM_PANCHAYAT','VILLAGE','CLUSTER') then array['LOCAL','DISTRICT','STATE','PLATFORM']
      when gk in ('SUB_DISTRICT','DISTRICT') then array['DISTRICT','STATE','PLATFORM']
      when gk='STATE_UT' then array['STATE','PLATFORM']
