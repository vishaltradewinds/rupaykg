-- Authorization lifecycle hardening.
-- Keep application lifecycle race-safe and align permission helpers with the
-- tenant-local permissions used by the authoritative onboarding role defaults.

create unique index if not exists stakeholder_applications_one_active_per_identity_idx
  on stakeholder_applications(identity_id)
  ...
  select exists (
    ...
    where om.identity_id = p_identity_id
      ...
      and (
        ...
        or r.permissions @> '["compliance.assess"]'::jsonb
        or r.permissions @> '["epr:manage"]'::jsonb
        or r.permissions @> '["epr.manage"]'::jsonb
      )
  );
$$;
  ...
  select exists (
    ...
    where om.identity_id = p_identity_id
      ...
      and (
        ...
        or r.permissions @> '["esg.write"]'::jsonb
        or r.permissions @> '["csr:manage"]'::jsonb
        or r.permissions @> '["csr.manage"]'::jsonb
      )
  );
$$;
